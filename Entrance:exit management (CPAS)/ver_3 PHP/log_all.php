...
if($_GET['page']=='search'){
  ...
  if(!empty($_POST['labo'])){
    $labo=$_POST['labo'];
    $_SESSION['s_labo'] = $_POST['labo'];
    $search[] = "labo like '%{$labo}%'";
  }
  if(!empty($_POST['room'])){
    $room=$_POST['room'];
    $_SESSION['s_room'] = $_POST['room'];
    $search[] = 'room = '.$room;  
  }
  if(!empty($_POST['att'])){
    $att=$_POST['att'];
    $_SESSION['s_attribute'] = $_POST['att'];
    if($att=="cps"){
  ...
  if( !empty($search) ) {
    ...
    $sql = 'select * from log where ' . $whereSQL;
  } else {
    $sql = 'select * from log';
  }
  $sth = $pdo->query($sql);
...
$sth = $pdo->prepare($sql);
$params = array(':id' => $_SESSION['id'], ':name' => $_SESSION['name'], ':labo' => $_SESSION['labo'], ':attribute' => $_SESSION['attribute'], ':status' => 'LOG', ':room' => 'ALL', ':seat' => $id.'/'.$name.'/'.$labo.'/'.$room.'/'.$att.'/'.$date, ':date' => date("Y/m/d H:i:s"));
$sth->execute($params);
?>
<!DOCTYPE html>
<html>
  ...
  <body>
    ...
    <section class="detail">
      ...
      <h3 class="d1">Date : <?php echo $date?></h3>
        <div class="room1">
            <input class="radio" type="button" name="room" value="ENTER or LEAVE" onclick="location.href='log.php?page=log'">
            <input class="radio_all" type="button" name="room" value="ALL" onclick="location.href='log_all.php?page=log'">
        </div>
